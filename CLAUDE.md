...
ARCHITECTURE.md or CHANGELOG.md, so read those. ARCHITECTURE.md is authoritative over
prose, and the code is authoritative over ARCHITECTURE.md.
...
1. **Operating-point drift**: committed calibration regenerates committed goldens
   ...
   a golden by hand, and never "fix" this gate by regenerating without the bump.
2. **Spec coverage** (`tests/test_spec.py`): every active `PMK-` ruling cited, every
   id-shaped string resolves. A new choice the design depends on gets a new ruling. An
   obsolete ruling is superseded (status change + successor). Editing or deleting a
...
Rulings of both kinds (spec rulings AND verdict rulings) are never edited into new
meanings: supersede with a new id.

## Honesty invariants (encoded in types and tests; keep them that way)

- A verdict is about the route label as served and says nothing about weights
  (PMK-CRT-002), and the no-weights sentence is golden-pinned. UNDETERMINED is a full
  verdict in its own right and never rounds to SAME-PRODUCER or SUBSTITUTED
  (PMK-RUL-001). UNAVAILABLE is never PASS, and an empty gate selection never passes
  (PMK-GTE-002).
...
`paper/` and `scratch/` are gitignored and never committed, and the sdist excludes them
a second time. `calibration/` and `validation/` are committed but excluded from the
sdist.
...
`tests/` fixtures are planted-truth synthetic archives. They are test fixtures and do
not constitute a benchmark. A case whose numbers cannot be checked on paper does not
belong in it.
