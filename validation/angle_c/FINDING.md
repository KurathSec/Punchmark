...
What was bought, and what it measures. Every number here comes from a file under
`derived/`, regenerated by the script each section names from the archives, and none
hand-edited. (Corrected 2026-08-22: this used to say every number came from
`angle_c_evaluation.json` and `collection_summary.json` alone, which was true before the
review-driven additions; those two remain the sources for C1 to C3 and the ledger.)
