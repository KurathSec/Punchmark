...
(`--task-as`): same-route per-ruling flag rates ran at 6-12x the declared 1% in three of eight
held-out (route, task) cells (see `validation/angle_a/derived/kt2.json`). A certificate's declared
